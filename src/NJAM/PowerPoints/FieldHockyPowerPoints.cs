...
{
    public class FieldHockeyPowerPoints : IType, IPowerPoints, IConvertible
    {
        ...
        public FieldHockeyPowerPoints(IUnitOfWork uow)
        {
            _uow = uow;
        }

        public FieldHockeyPowerPoints(IUnitOfWork uow, int TeamId, List<VsandGameReport> oGameReports, DateTime dStart, DateTime dEnd)
        {
            _uow = uow;
        ...
        public FieldHockeyPowerPoints(IUnitOfWork uow, int TeamId, List<VsandGameReport> oGameReports, DateTime dStart, DateTime dEnd, int maxGames)
        {
            _uow = uow;
        ...
        public double TieBreak()
        {
            ...
            IEnumerable<VsandGameReport> oEligibleGames = this.GameReports.Where(gr => gr.Deleted == false && gr.Final == true && gr.Teams.Count == 2).OrderBy(gt => gt.GameDate).Take(iIncludeGames);
            ...
            foreach (VsandGameReport oGR in oEligibleGames)
            {
                ...
                if (oOpp.Team != null)
                {
                    ...
                    if (!bOutOfState)
                    {
                        ...
                        var oData = _uow.GameReports.List(
                                                        g => g.Teams.Any(gt => gt.Team.TeamId == iOppTeamId) && g.Deleted == false && g.Final == true && g.Teams.Count == 2 && g.GameDate >= this.StartDate && g.GameDate <= this.EndDate,
                                                        x => x.OrderBy(g => g.GameDate),
                                                        new List<string> { "Teams" }
                                                    ).Result.Take(this.IncludeGames);
