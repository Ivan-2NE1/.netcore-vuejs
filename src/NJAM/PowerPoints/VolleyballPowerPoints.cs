...
{
    public class VolleyballPowerPoints : IType, IPowerPoints, IConvertible
    {
        ...
        public VolleyballPowerPoints(IUnitOfWork uow)
        {
            _uow = uow;
        }

        public VolleyballPowerPoints(IUnitOfWork uow, int TeamId, List<VsandGameReport> oGameReports, DateTime dStart, DateTime dEnd)
        {
            _uow = uow;
        ...
        public VolleyballPowerPoints(IUnitOfWork uow, int TeamId, List<VsandGameReport> oGameReports, DateTime dStart, DateTime dEnd, int maxGames)
        {
            _uow = uow;
                ...
                if (oGameTeams.Count == 2)
                {
                    ...
                    if (oOpp.Team != null)
                    {
                        ...
                        if (iOppWins > 0 || iOppLosses > 0 || iOppTies > 0)
                        {
                            if (bOutOfState)
                            {
                                sRecMsg = " <small>OOS</small>";
                            }
                            else
                            {
                                sRecMsg = " <small>Write-In</small>";
                            }
                        ...
                        else
                        {
                            ...
                            oOppWinEx = oOpp.Team.CustomCodes.FirstOrDefault(gt => gt.CodeName == "OOSFinalWins");
                            if (oOppWinEx != null)
                            {
                                string sOppWins = oOppWinEx.CodeValue;
                                int.TryParse(sOppWins, out iOppWins);
                            }
                            oOppLoseEx = oOpp.Team.CustomCodes.FirstOrDefault(gt => gt.CodeName == "OOSFinalLosses");
                            if (oOppLoseEx != null)
                            {
                                string sOppLosses = oOppLoseEx.CodeValue;
                                int.TryParse(sOppLosses, out iOppLosses);
                            }
                            oOppTieEx = oOpp.Team.CustomCodes.FirstOrDefault(gt => gt.CodeName == "OOSFinalTies");
                            if (oOppTieEx != null)
                            {
                                string sOppTies = oOppTieEx.CodeValue;
                                int.TryParse(sOppTies, out iOppTies);
                        ...
                        if (!int.TryParse(sOppGroup, out iGroupVal))
                        {
                            switch (sOppGroup.ToLower())
                            {
                                ...
                                default:
                                    {
                                        string sGroup = sOppGroup.Replace("group", "").Trim();
                        ...
                        if (!bOutOfState)
                        {
                            ...
                            var oData = _uow.GameReports.List(
                                                            g => g.Teams.Any(gt => gt.Team.TeamId == iOppTeamId) && g.Deleted == false && g.Final == true && g.GameDate >= this.StartDate && g.GameDate <= this.EndDate && g.PPEligible == true,
                    ...
                    oCalcSb.AppendLine("<tr><td class=\"center\">" + oGR.GameDate.ToString("%M/%d/yy") + "</td><td>" + oOpp.TeamName + " (" + iOppRecordWins + "-" + iOppRecordLosses + "-" + iOppRecordTies + sRecMsg + ")</td><td>" + sResult + "</td><td class=\"center\">" + iQualityPoints + "</td><td class=\"center\">" + iGroupPoints + "</td><td class=\"center\">" + dResidualPoints + "</td><td class=\"center\">" + dPPVal + "</td></tr>");
                ...
                if (oGameTeams.Count == 2)
                {
                    ...
                    if (oOpp.Team != null)
                    {
                        ...
                        if (!bOutOfState)
                        {
                            ...
                            var oData = _uow.GameReports.List(
                                                            g => g.Teams.Any(gt => gt.Team.TeamId == iOppTeamId) && g.Deleted == false && g.Final == true && g.Teams.Count == 2 && g.GameDate >= this.StartDate && g.GameDate <= this.EndDate && g.PPEligible == true,
                                                            x => x.OrderBy(g => g.GameDate),
                                                            new List<string> { "Teams" }
                                                        ).Result.Take(this.IncludeGames);
        ...
        {
            throw new InvalidCastException("NJAM.PowerPoints.VolleyballPowerPoints cannot be converted to Boolean");
        }
