...
namespace VSAND.Backend.TagHelpers
{
    ...
    /// </summary>
    //[UsedImplicitly]
    [HtmlTargetElement(Attributes = "show-if-active")]
    public class VisibleRouteHelper : ActiveRouteTagHelper
    {
        /// <summary>
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            output.Attributes.RemoveAll("show-if-active");

            // Local function to determine if the attached resource should be hidden or not
            bool ShouldBeHidden() => !(output.Attributes[ClassAttribute]?.Value.ToString().Contains(DefaultActiveClass)).GetValueOrDefault();
        }
    }
}
