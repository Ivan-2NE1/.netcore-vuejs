    ...
    export default {
        ...
        methods: {
            ...
            getEventPlayerStatValue(refStat) {
                if (this.eventResult.players === undefined || this.eventResult.players === null) {
                    return "";
                }

                let statValueRaw = 0;
                let player = this.eventResult.players[0];
                if (player !== undefined && player !== null) {
                    let stat = player.stats.find(s => s.statId === refStat.sportEventStatId);
