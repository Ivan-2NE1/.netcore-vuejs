<template>
    <widget-wrapper icon="users" title="Choose a Game Type"
        ...
        <template v-slot:footer>
            <router-link to="scheduleGame" tag="button" class="btn btn-primary btn-lg" v-bind:disabled="selectedEventType === null || selectedEventType.id <= 0">
                Continue
            </router-link>
        </template>
    </widget-wrapper>
...
<script>
    ...
    export default {
        ...
        computed: {
            sportId() {
                return this.$store.state.scheduleGame.sportId;
            },

            scheduleYearId() {
                return this.$store.state.scheduleGame.scheduleYearId;
            },
        ...
        mounted() {
            ...
            document.addEventListener('keydown', function (event) {
                if (event.code == "Enter") {
                    ...
                    vm.$router.push({ name: 'scheduleGame', params: {} });
                }
            });
        }
    }
