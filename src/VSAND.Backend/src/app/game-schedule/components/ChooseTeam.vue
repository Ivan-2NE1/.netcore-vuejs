<template>
    <widget-wrapper icon="users" title="Choose a Team"
        ...
        <template v-slot:footer>
            <router-link to="chooseEventType" tag="button" class="btn btn-primary btn-lg" v-bind:disabled="teamId === null || teamId <= 0">
                Continue
            </router-link>
        </template>
    </widget-wrapper>
...
<script>
    ...
    export default {
        ...
        computed: {
            teamId() {
                return this.$store.state.scheduleGame.refTeamId;
            },
